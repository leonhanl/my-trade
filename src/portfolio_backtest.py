...
def main():
    ...
    # backtest.run_rebalance_backtest('ANNUAL_REBALANCE') # 每年1月1日进行再平衡，恢复到预设的目标比例
    ...
    # for year, return_rate in portfolio_return_analysis['annual_returns'].items():
    #     print(f"{year}年收益率: {return_rate*100:.2f}%")

    # 计算组合中每个资产的收益率
    # asset_returns = backtest.calculate_asset_return()
    # print("\n资产收益率:")
    # for symbol, return_rate in asset_returns['asset_returns'].items():
    #     print(f"{symbol} {TRADING_PRODUCTS[symbol]['name']}总收益率: {return_rate*100:.2f}%")
    #     print(f"{symbol} {TRADING_PRODUCTS[symbol]['name']}年化收益率: {asset_returns['annulized_asset_returns'][symbol]*100:.2f}%")
    ...
    for i, drawdown in enumerate(max_drawdowns, 1):
        ...
        #     print(f"恢复日期: {drawdown['recovery_date']}")
        #     print(f"恢复天数: {drawdown['recovery_length']}")
        # print(f"回撤持续天数: {drawdown['drawdown_length']}")
